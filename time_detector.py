...
def angle_time(time):
    """
    :param time: hour or minute
    :return: The range of angles (between the arm and the axis of the person) in which the arm should rest.
    """
    angle_step = 360 // 12
...
def hand_control(hour, minute):
    """
    Mapping the hand to the arrows on the screen
    """
    message = ':-('
    if 1 <= minute <= 5:
...
def detect(image, loc_hour, loc_minute, x_med, y_med, x_nose, y_nose, x_r_hand, y_r_hand, x_l_hand, y_l_hand, draw_arrow=True):
    """
    Checking the correct position of the hands
    """
    done = False
